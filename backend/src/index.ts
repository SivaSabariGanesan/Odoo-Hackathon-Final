...
// Drizzle returns bigserial/bigint columns as native BigInt.
// JSON.stringify cannot handle BigInt out of the box — patch it globally.
...
// ─── Event emitter ────────────────────────────────────────────────────────────
// registerEmitter() must be called here once Siva's Socket.IO / WS module ships.
// Until then, events fall back to a structured console.log (see utils/events.ts).
// Example (uncomment when sivaEventClient is available):
//
//   import { sivaEventClient } from "./lib/sivaEventClient.ts";
//   import { registerEmitter } from "./utils/events.ts";
//   sivaEventClient.on("ready", () => {
//     registerEmitter(sivaEventClient.emit.bind(sivaEventClient));
//   });

...
function CustomerModal({
  ...
  async function handleSave() {
    ...
    try {
      let saved: Customer;
      if (customer) {
        saved = await updateCustomer(customer.publicId, { name: name.trim(), email: email || undefined, phone: phone || undefined });
      } else {
        saved = await createCustomer({ name: name.trim(), email: email || undefined, phone: phone || undefined });
      }
      onSave(saved);
      onClose();
    } catch (e: any) {
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm border border-gray-200 overflow-hidden">
        ...
        <div className="px-5 py-4 space-y-3">
          ...
          <div className="relative">
            ...
            <input value={name} onChange={e => setName(e.target.value)} placeholder="e.g Eric Smith"
          ...
          <div className="relative">
            ...
            <input value={email ?? ""} onChange={e => setEmail(e.target.value)} placeholder="eric@odoo.com" type="email"
              className="w-full pl-9 pr-4 py-2.5 text-sm border border-gray-200 rounded-xl outline-none focus:border-[#714B67] transition text-[#121B35] placeholder:text-gray-300" />
          </div>
          <div className="relative">
            <Phone className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
            <input value={phone ?? ""} onChange={e => setPhone(e.target.value)} placeholder="+91 9898989898" type="tel"
              className="w-full pl-9 pr-4 py-2.5 text-sm border border-gray-200 rounded-xl outline-none focus:border-[#714B67] transition text-[#121B35] placeholder:text-gray-300" />
          </div>
        </div>
        ...
        <div className="border-t border-gray-100">
          ...
          {customer && (
            <button onClick={handleDelete} disabled={deleting}
              className="w-full py-3 text-sm font-bold text-white bg-red-500 hover:bg-red-600 transition flex items-center justify-center gap-2 disabled:opacity-50">
...
export default function Customers() {
  ...
  const loadCustomers = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await fetchCustomers({ pageSize: 200 });
      setCustomers(data);
    } catch {
  ...
  function handleSave(c: Customer) {
    setCustomers(prev => {
      const exists = prev.find(x => x.publicId === c.publicId);
      return exists ? prev.map(x => x.publicId === c.publicId ? c : x) : [c, ...prev];
    });
  }

  function handleDelete(id: string) {
    setCustomers(prev => prev.filter(c => c.publicId !== id));
  }
    ...
    <div className="min-h-screen bg-[#F5F5F7] flex flex-col">
      ...
      <div className="w-full max-w-lg">
        <div className="flex items-center gap-2 mb-5">
          ...
          <h1 className="text-xl font-bold" style={{ color: "#121B35" }}>Customers</h1>
        </div>

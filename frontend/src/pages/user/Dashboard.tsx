...
import { fetchKpis } from "../../api/reports";
import { listOrders, type Order } from "../../api/orders";
import { fetchLastClosedSession } from "../../api/sessions";
...
export default function UserDashboard() {
  ...
  const [session,      setSession]      = useState<{ name: string; openedAt: string; cashier: string } | null>(null);

  const loadData = useCallback(async () => {
    try {
      const [kpiData, orderData] = await Promise.all([
        fetchKpis({ period: "today" }),
        listOrders({ pageSize: 4 }),
      ]);
      setKpis(kpiData);
      setRecentOrders(orderData);
    } catch { /* fail gracefully */ }

    // Try to get active session info (last closed gives us context)
    try {
      const s = await fetchLastClosedSession();
      if (s) {
        setSession({
          name: `Session ${s.publicId.slice(0, 8)}`,
          openedAt: new Date(s.openedAt).toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit" }),
          cashier: s.openedBy?.name ?? "Staff",
        });
      }
    } catch { /* ignore */ }
  }, []);
  ...
  const todayStats = [
    { label: "Orders Taken",    value: String(kpis.totalOrders), icon: ClipboardList,     iconBg: "bg-blue-50",      iconColor: "text-blue-600"    },
    { label: "Total Sales",     value: `₹${parseFloat(kpis.revenue).toLocaleString("en-IN")}`, icon: DollarSign, iconBg: "bg-emerald-50", iconColor: "text-emerald-600" },
    { label: "Avg Order Value", value: `₹${parseFloat(kpis.avgOrderValue).toLocaleString("en-IN")}`, icon: TrendingUp, iconBg: "bg-amber-50", iconColor: "text-amber-600" },
    { label: "Tables Served",   value: "—", icon: MonitorSmartphone, iconBg: "bg-[#714B67]/10",   iconColor: "text-[#714B67]"   },
  ];

  const sessionElapsed = session ? elapsedSince(new Date(Date.now() - 3_600_000).toISOString()) : "—";
        ...
        <div className="bg-[#714B67] rounded-2xl p-4 sm:p-5 text-white shadow-md shadow-[#714B67]/20">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="text-lg font-bold">{session?.name ?? "POS Session"}</p>
              <p className="text-sm text-white/70 mt-0.5">
                {session ? `Opened at ${session.openedAt} · ${session.cashier}` : "No session info"}
              </p>
            </div>
          ...
          <div className="flex items-center gap-3 mt-4 pt-3 border-t border-white/20">
            <div className="ml-auto">
              <Link to={ROUTES.POS_SESSION}
                ...
                <ArrowUpFromLine className="w-3 h-3" />Close Session
              </Link>
            </div>
          </div>
        </div>

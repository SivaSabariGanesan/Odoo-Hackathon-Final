...
function ProductForm({
  ...
  async function handleSave() {
    ...
    try {
      let saved: Product;
      if (product) {
        saved = await updateProduct(product.publicId, {
          name: name.trim(),
          categoryId,
    ...
    <>
                ...
                <div className="relative" ref={dropRef}>
                  ...
                  {catDropdown && (
                    <div className="absolute left-0 top-full mt-1 bg-white border border-gray-200 rounded-xl shadow-xl z-30 w-full py-1">
                      {categories.map(cat => (
                        <button key={cat.publicId}
                          onClick={() => { setCategoryId(cat.publicId); setCatDropdown(false); }}
      ...
      {catPopup && (
        <QuickCategoryModal
          ...
          onSaved={cat => { onCategoryCreated(cat); setCategoryId(cat.publicId); }}
        />
      )}
    </>
  );
...
export default function Products() {
  ...
  async function handleBulkArchive() {
    try {
      ...
      toast.success(`${selected.size} product(s) archived`);
      setSelected(new Set());
      load();
    } catch (e: any) {
      toast.error(e?.response?.data?.error?.message ?? "Failed to archive");
    }
  }
  ...
  function handleSaved(p: Product) {
    setProducts(prev =>
      prev.find(x => x.publicId === p.publicId)
        ? prev.map(x => x.publicId === p.publicId ? p : x)
        : [p, ...prev]
    );
  }
                      ...
                      return (
                        <tr key={p.publicId}
                          ...
                          <td className="px-4 py-3.5" onClick={e => e.stopPropagation()}>
                            <input type="checkbox" checked={selected.has(p.publicId)} onChange={() => toggleSelect(p.publicId)}
                              className="accent-[#714B67] w-3.5 h-3.5 cursor-pointer" />
                          </td>
